...
**쿼리 포맷 (v1.1.4, 260428 보강):**
```
in:<P1채널1> in:<P1채널2> {customer_token} after:<edu_start - 60일 YYYY-MM-DD>
```

- `customer_token`: 1차 회사명 추출 (아래 v1.1.4 결정 룰 참조). **과정명 토큰은 1차 검색에서 제외** — 세일즈맵 deal_name과 운영 채널 코스명이 다를 수 있음 (260428 발견: deal `Customer B-AI 시너지` vs ops thread `Customer B (full)_바이브코딩 과정`)
...
**v1.1.4 (260428) — customer_token 결정 + 변형 검색 룰 (필수):**

> 배경: v1.1.3 명세에 Customer B BOT thread `1770855296.526319`가 핵심 소스로 명시돼 있었음에도 실 실행에서 미수집. 원인은 deal_name 토큰(`Customer B`)으로만 검색해 BOT 코스명(`Customer B (full)_바이브코딩`)을 놓쳤기 때문. 1회 검색으로 끝내지 말고 다음 변형 시퀀스를 강제 수행.

1. **1차 — deal_name 분해**
   - `deal_name`이 `<회사명>-<과정명>` 또는 `<회사명>_<과정명>` 형태면 구분자 앞 회사명만 추출
   - 예: `Customer B-AI 시너지 과정` → `Customer B`
   - 예: `Customer A_2026 AX 교육` → `Customer A`

2. **2차 — 회사명 어순 변형 (한국어 회사명에 흔함)**
   - 1차 검색 결과 BOT 정형 thread (7-2.1 패턴) **0건이면 반드시 시도**
   - 예: `Customer B` → `Customer B (variant)` (실제 워크플로 등록 표기)
   - 예: `Customer A (Korean)` ↔ `Customer A` (한글·영문 표기 변형)
   - 두 토큰을 별도 검색 후 결과 합산

3. **3차 — 핵심 토큰만 (마지막 fallback)**
   - 1·2차 모두 0건이면 회사명에서 가장 식별력 높은 토큰만 (예: `Customer A 단축 토큰`, `Customer B 단축 토큰`)
   - false positive 위험 ↑ → 7-2.1 본문 검증 강하게 적용

**중요 — 부분 매치 채택 강제:**

BOT 정형 thread는 코스명에 회사명이 결합 형태로 들어 있는 경우가 흔함 (`Customer B (full)_바이브코딩 과정`, `Customer A_2026 AI 교육`). **검색어와 BOT 본문 코스명 사이 부분 매치만 돼도 채택**해야 함 — 정확 일치 강제 금지. (예: 검색어 `Customer B (variant)` ↔ 코스명 `Customer B (full)_바이브코딩` 매치 OK)
...
1. **고객사명 직접 등장 확인** (필수)
   ...
   - **부분 매치 허용 (v1.1.4)**: BOT 정형 thread의 `코스명:` 필드는 `<회사명변형>_<과정명>` 결합인 경우가 흔함. 검색어가 코스명의 일부 토큰과 일치하면 채택 (예: 검색 `Customer B (variant)` ↔ 코스명 `Customer B (full)_바이브코딩 과정` 매치 OK)
   - 정확 일치 안 해도 회사명 어순 변형(예: Customer B ↔ Customer B (variant)) 허용
   - 위 모두 해당 안 되면 → false positive로 제외
...
- Customer B BOT thread `1770855296.526319` (2/12, 부모 "1차 4/6~8 ... 8차 6/30~7/8" 8차수 정형) ⭐ 핵심 소스
- Customer A BOT thread `1772069983.632159` (2/26, 답글에 차수 분해) ⭐ 핵심 소스
- 위 BOT thread를 LLM이 검색 결과에서 false positive로 제외 → 자유 서술형만 채택 → 정규식 매칭 0건 → 다차수 분해 lump
- → BOT 정형 thread 최우선 채택 룰로 차단

**260428 v1.1.4 추가 실측 — 명세 명시 후에도 실 실행 미수집 사례:**
- Customer B: v1.1.3 명세에 `1770855296.526319` 핵심 소스로 명시했으나 실 `/운영일지` 실행에서 미수집. Customer A(`1772069983`)는 정상 수집됨. 차이는 검색 키워드:
  - Customer A: deal_name `Customer A_2026 AX 교육`, 워크플로 코스명 `Customer A_2026 AI 교육` → `Customer A` 토큰 정확 일치
  - Customer B: deal_name `Customer B-AI 시너지 과정`, 워크플로 코스명 `Customer B (full)_바이브코딩 과정` → `Customer B`로는 매치 안 됨 (회사명 어순 다름 + 회사명 표기 다름)
- → v1.1.4: deal_name 분해 후 **회사명 어순 변형** 검색 강제 (1차 0건 시 자동으로 `Customer B (variant)`까지 시도). 본문 검증에서도 부분 매치·어순 변형 채택 허용.
...
- 부모 본문이 `<시트 링크>`나 단순 알림(`@OwnerName Customer A_2026 AX 교육`)만 있어도 답글에 차수 분해 정형 데이터(`기초: 4월 6, 8 / 5월 11, 12`) 들어있는 경우 多
...
- **검색 윈도우는 7-2의 `edu_start - 60일` 적용** (260428 v1.1.4 — 옛 "30일 이내" 룰 폐기. BOT 정형 thread는 운영 시작 1~3개월 전 작성이 흔하므로 60일 미만 윈도우는 핵심 소스 누락)
- 매칭 0건 시 → 7-2의 변형 검색(2·3차) 시도 후, 그래도 0건이면 해당 deal에 빈 `threads: []`로 저장 (compose_schedule이 폴백 처리)
