...
### ⚠️ 강제 분기 알고리즘 (v1.1.3, 260428 — 정통 흐름 미파싱 차단)

**필수 절차 (우회 금지):**
1. **mimeType 확인 먼저** — `search_drive_files` 또는 `get_drive_file_*` 결과의 `mimeType` 필드 확인
2. **mimeType별 분기 강제 결정** — 아래 분기 표대로
3. **❌ 절대 금지: 분기 확인 없이 `get_spreadsheet_info` 우선 호출** — `.xlsx`이면 API가 거부하고, 그 후 fallback 명세 따라가지 않으면 LLM이 "메타만 반영" 자의적 처리로 빠짐 (260428 실측에서 유한킴벌리 26회차 미파싱 → 다차수 분해 0건 발생)

| `mimeType` 값 | 분기 | 우선 호출 도구 |
|---|---|---|
| `application/vnd.google-apps.spreadsheet` | Google Sheets | `get_spreadsheet_info` |
| `application/vnd.openxmlformats-officedocument.spreadsheetml.sheet` | Excel `.xlsx` | **`get_drive_file_content`** ⭐ |
| 기타 (PDF·docx 등) | 폴백 | `get_drive_file_content` |
...
- **`get_drive_file_content` 1회 호출** (자동 파싱, 모든 탭 텍스트 추출) — **`.xlsx`에 `get_spreadsheet_info` 호출 X**
- 탭 분리는 내부적으로 이미 됨
- 내용 추출 후 탭 whitelist로 필터링
- 추출된 텍스트 그대로 STEP 6 LLM 파싱 입력으로 사용
...
### ⚠️ 출력 스키마 강제 (v1.1.2, 260428 — LLM 자유 생성 차단)

**필수 메타 필드 (모든 타입 공통, LLM이 임의 변경·생략 금지):**
- `type` (str) — "기획문서" / "견적서" / "싱크업" / "강의준비문서" / "싱크업 문서"
- `lifecycle_stage` (str) — "draft" / "in_progress" / "preparation" / "active" / null
- `tabs_read` (list) — 실제로 읽은 탭 이름 목록
- `parsed` (object) — 타입별 항목 (아래 스펙대로)

**`session_schedule[]` 추출 절대 룰 (260428 보강):**
1. **모든 회차 빠짐없이 포함** — LLM이 sample만 추출하거나 임의 생략 금지. 시트의 모든 데이터 행이 session_schedule 항목으로 들어가야 함.
2. **필수 필드 강제:** `{session_no, date, track, instructor, edu_start, edu_end}` — 일부 누락된 행도 가능한 필드만 채우고 항목 자체는 보존
3. **session_no 자동 부여** — 시트에 명시 없어도 트랙별 또는 통합 일련번호로 1부터 부여
4. **트랙별 분리 시 통합 sno** — 다중 트랙(영업/전사 등)이면 트랙별로 따로 1·2·3, 또는 전체 통합 1·2·3 일관 적용
5. **강사명 시트 그대로** — LLM이 약어·변형·임의 보정 금지 (실제 시트의 강사명 칼럼 값 그대로)

**참고: 260428 실측 — 같은 파일을 다른 세션이 파싱했을 때:**
- 어제: `parsed` 12개 키 (ld_name·tracks·session_schedule 26건 등)
- 오늘: `parsed` 5개 키 (session_schedule 23건 — 3건 누락 + session_no 없음)
- **같은 raw인데 LLM이 다르게 출력** → 위 스키마 강제 필수.
